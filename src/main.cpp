...
int main(int argc, char* argv[]) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if(tutorial||game_over){
            ...
            // https://www.tug.org/pracjourn/2007-4/walden/color.pdf
            glClearColor(0.1f * sin(curTime) + 0.2f, 0.1f * sin(curTime+2) + 0.2f, 0.1f * sin(curTime) + 0.3f, 1.0f);
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        }
        ...
        if(tutorial) {
            ...
            TextRendering_PrintString(window, "              >> ANDROMEDA ANIMAL RESCUE <<            " , -1.15, 0.3, 2);
            TextRendering_PrintString(window, "             Use WASD to control the player            " , -0.525, 0.2);
            TextRendering_PrintString(window, "Planes  are worth 100 points, missing them costs you 50" , -0.525, 0.1);
            TextRendering_PrintString(window, "Bunnies are worth 500 points                           " , -0.525, 0.0);
            TextRendering_PrintString(window, "Cows    are worth 300 points                           " , -0.525, -0.1);
            TextRendering_PrintString(window, "     Don't hit the planets! It will cost you a life    " , -0.525, -0.2);
            TextRendering_PrintString(window, "       If you miss too many planes you will die!       " , -0.525, -0.3);
